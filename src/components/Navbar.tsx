...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab, stats, onSwitchToOwnerView, onAuth }) => {
  ...
  return <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs">
    <OwnerEditable request={{ title: 'الشريط الإعلاني', fields: [{ key: 'announcementBanner', label: 'إعدادات الإعلان (JSON)', kind: 'json', help: 'enabled, textAr, badgeTextAr, linkUrl' }] }}>{settings.announcementBanner?.enabled ? (
    ...
    ) : ownerEditEnabled ? <div className="bg-amber-50 text-amber-800 text-xs text-center py-2 font-bold">الشريط الإعلاني معطّل — اضغط لتحريره</div> : null}</OwnerEditable>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="flex items-center justify-between h-16 md:h-20">
      <OwnerEditable request={{ title: 'هوية رأس الموقع', fields: [{ key: 'siteName', label: 'اسم المنصة' }, { key: 'logoUrl', label: 'الشعار الكامل', kind: 'image' }, { key: 'faviconUrl', label: 'أيقونة الموقع', kind: 'image' }, { key: 'taglineAr', label: 'الوصف المختصر' }] }}><div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab('home')}>{settings.logoUrl?<img src={settings.logoUrl} alt={settings.siteName} className="w-10 h-10 rounded-xl object-contain"/>:<div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-black text-xl">30</div>}<div><span className="font-black text-2xl text-slate-800 font-english">{settings.siteName || 'English30'}</span><p className="text-[11px] text-slate-500 hidden sm:block">{settings.taglineAr || 'تعلم الإنجليزية بذكاء ونظام'}</p></div></div></OwnerEditable>
