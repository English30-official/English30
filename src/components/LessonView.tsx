...
export const LessonView: React.FC<LessonViewProps> = ({ lesson: providedLesson, courseId, setActiveTab, onLessonCompleted }) => {
  ...
  const [speakingId, setSpeakingId] = useState<string|null>(null); const [fibAnswers,setFibAnswers]=useState<Record<string,string>>({}); const [showFibResults,setShowFibResults]=useState(false); const [finalAnswers,setFinalAnswers]=useState<Record<string,number>>({});
  ...
  return <div className="max-w-4xl mx-auto space-y-8 py-6" dir="rtl">
    ...
    {activeStep==='quiz'&&<section className="bg-white p-7 rounded-3xl border space-y-5"><h2 className="text-xl font-black">اختبار الدرس</h2><DatabaseQuizPanel lessonId={lesson.id}/>{lesson.quizQuestions.map((q:QuizQuestion)=><div key={q.id} className="space-y-2"><p className="font-bold">{q.questionAr||q.questionEn}</p>{q.questionEn&&<p className="font-english text-sm text-slate-500">{q.questionEn}</p>}{q.options.map((o,j)=><button key={j} onClick={()=>quiz.selectAnswer(q.id,j)} className={`block w-full text-right p-3 rounded-xl border text-sm ${quiz.answers[q.id]===j?'border-indigo-600 bg-indigo-50':''}`}>{o}</button>)}</div>)}{lesson.quizQuestions.length>0&&<button onClick={()=>quiz.setShowResults(true)} className="bg-slate-700 text-white px-5 py-3 rounded-xl font-bold">نتيجة التمرين القديم ({quiz.score}/{lesson.quizQuestions.length})</button>}</section>}
