...
import { ActiveTab, Lesson, VocabItem, SentenceItem, FillInBlankQuestion } from '../types';
import { entitlementService, lessonsService, progressService, settingsService } from '../services';
...
export const LessonView: React.FC<LessonViewProps> = ({ lesson: providedLesson, courseId, setActiveTab, onLessonCompleted }) => {
  ...
  const [quizzesEnabled, setQuizzesEnabled] = useState(settingsService.getSettingsSync().featureFlags?.quizzes !== false);
  const [loading, setLoading] = useState(!providedLesson); const [loadError, setLoadError] = useState('');
  const [activeStep, setActiveStep] = useState<'explanation'|'content'|'video'|'vocab'|'sentences'|'grammar'|'fill-blank'|'quiz'|'final-test'|'completed'>('explanation');
  const [speakingId, setSpeakingId] = useState<string|null>(null); const [fibAnswers,setFibAnswers]=useState<Record<string,string>>({}); const [showFibResults,setShowFibResults]=useState(false);
  const [progressMessage,setProgressMessage]=useState('');

  useEffect(()=>settingsService.subscribe((value)=>setQuizzesEnabled(value.featureFlags?.quizzes!==false)),[]);
  useEffect(()=>{if(!quizzesEnabled&&(activeStep==='quiz'||activeStep==='final-test'))setActiveStep('explanation');},[quizzesEnabled,activeStep]);
  ...
  useEffect(()=>{
    if(!lesson)return;
    void progressService.saveLessonProgress({lessonId:lesson.id}).catch(()=>undefined);
  ...
  const finish=async()=>{
    if(!lesson)return;
    try {
      const progress=await progressService.saveLessonProgress({lessonId:lesson.id,isCompleted:true});
      if(!progress.isCompleted){
        setProgressMessage(`يكتمل الدرس بعد دراسة 80% من مدته. تقدمك الموثق الآن ${Math.round(progress.watchPercentage)}%.`);
        return;
      }
      await progressService.syncCourseProgress(lesson.courseId);
    } catch(error) {
      setProgressMessage(error instanceof Error?error.message:'تعذر حفظ تقدم الدرس.');
      return;
    }
  ...
  const steps:[typeof activeStep,string][]=[['explanation','📖 الشرح'],...(lesson.blocks?.length ? [['content','🧩 محتوى الدرس'] as [typeof activeStep,string]] : []),['video','🎬 الفيديو'],['vocab',`🔤 المفردات (${lesson.vocabList.length})`],['sentences','💬 الجمل'],['grammar','📐 القاعدة'],['fill-blank','✍️ التدريب'],...(quizzesEnabled ? [['quiz','📝 الاختبار الرسمي'] as [typeof activeStep,string]] : []),['final-test','✅ إكمال الدرس']];
  return <div className="max-w-4xl mx-auto space-y-8 py-6" dir="rtl">
    ...
    {activeStep==='content'&&<section><LessonBlocksRenderer blocks={lesson.blocks ?? []}/></section>}
    {activeStep==='video'&&<section className="bg-slate-950 rounded-3xl overflow-hidden"><div className="aspect-video flex items-center justify-center text-white">{lesson.videoUrl?<LessonVideoPlayer url={lesson.videoUrl} title={lesson.titleAr}/>:<div className="text-center"><Film className="w-12 h-12 mx-auto text-indigo-400 mb-3"/><p className="font-bold">لم يُرفع فيديو لهذا الدرس بعد</p><p className="text-xs text-slate-400 mt-1">يمكن للمالك رفع الفيديو من لوحة التحكم.</p></div>}</div></section>}
    ...
    {activeStep==='quiz'&&<section className="bg-white p-7 rounded-3xl border space-y-5"><h2 className="text-xl font-black">اختبار الدرس</h2><DatabaseQuizPanel lessonId={lesson.id}/></section>}
    {activeStep==='final-test'&&<section className="bg-white p-7 rounded-3xl border space-y-5"><h2 className="text-xl font-black">إكمال الدرس</h2><p className="text-sm text-slate-600">يُحسب التقدم داخل قاعدة البيانات بحسب مدة الدراسة الفعلية، ولا يعتمد على نتيجة يرسلها المتصفح.</p>{progressMessage&&<div className="p-3 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-sm">{progressMessage}</div>}<button onClick={()=>void finish()} className="bg-emerald-600 text-white px-5 py-3 rounded-xl font-bold"><Play className="inline w-4 h-4 ml-1"/>تسجيل إكمال الدرس</button></section>}
