...
export const LessonView: React.FC<LessonViewProps> = ({ lesson: providedLesson, courseId, setActiveTab, onLessonCompleted }) => {
  ...
  return <div className="max-w-4xl mx-auto space-y-8 py-6" dir="rtl">
    ...
    {activeStep==='quiz'&&<section className="bg-white p-7 rounded-3xl border space-y-5"><h2 className="text-xl font-black">اختبار الدرس</h2>{lesson.quizQuestions.map((q:QuizQuestion)=><div key={q.id} className="space-y-2"><p className="font-bold">{q.questionAr||q.questionEn}</p>{q.questionEn&&<p className="font-english text-sm text-slate-500">{q.questionEn}</p>}{q.options.map((o,j)=><button key={j} onClick={()=>quiz.selectAnswer(q.id,j)} className={`block w-full text-right p-3 rounded-xl border text-sm ${quiz.answers[q.id]===j?'border-indigo-600 bg-indigo-50':''}`}>{o}</button>)}</div>)}<button onClick={()=>quiz.setShowResults(true)} className="bg-indigo-600 text-white px-5 py-3 rounded-xl font-bold">إظهار النتيجة ({quiz.score}/{lesson.quizQuestions.length})</button></section>}
