...
import { Activity, Award, Blocks, BookOpen, ClipboardCheck, CreditCard, FileJson, Files, FileText, Flag, GraduationCap, HelpCircle, History, Images, KeyRound, LayoutDashboard, LayoutTemplate, Loader2, Megaphone, Settings, ShieldCheck, Sparkles, Users } from 'lucide-react';
...
const OwnerHomepageBuilder = lazy(() => import('./OwnerHomepageBuilder').then((module) => ({ default: module.OwnerHomepageBuilder })));
const OwnerCampaignManager = lazy(() => import('./OwnerCampaignManager').then((module) => ({ default: module.OwnerCampaignManager })));
...
export const OwnerDashboard: React.FC<OwnerDashboardProps> = ({ onSwitchToStudentView }) => {
  ...
  const items: NavItem[] = [
    { id:'overview',label:'النظرة العامة',icon:LayoutDashboard }, { id:'homepage',label:'الصفحة الرئيسية المرئية',icon:LayoutTemplate,permission:'homepage.manage',highlight:true }, { id:'campaigns',label:'الحملات الموسمية',icon:Megaphone,permission:'campaigns.manage',highlight:true }, { id:'courses',label:'الدورات والدروس',icon:BookOpen,permission:'content.manage' },
