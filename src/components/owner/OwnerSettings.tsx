  ...
  return (
    <div className="space-y-8 animate-fadeIn" dir="rtl">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-2xs space-y-5">
          <div className="border-b border-slate-100 pb-3"><h3 className="font-extrabold text-base text-slate-900">الهوية اللونية والتصميم الآمن</h3><p className="text-xs text-slate-500 mt-1">تحكم محدود وآمن بالألوان والقوالب دون CSS مخصص أو حقن شيفرة.</p></div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">{([['primaryColor','اللون الأساسي'],['secondaryColor','اللون الثانوي'],['accentColor','لون التمييز'],['backgroundColor','لون الخلفية']] as const).map(([key,label])=><label key={key} className="text-xs font-bold">{label}<div className="mt-2 flex items-center gap-2"><input type="color" value={settings.theme?.[key] || '#4f46e5'} onChange={(e)=>setSettings({...settings,theme:{...settings.theme,[key]:e.target.value}})} className="w-10 h-10 rounded-lg border p-0.5"/><input dir="ltr" value={settings.theme?.[key] || ''} onChange={(e)=>/^#[0-9a-f]{6}$/i.test(e.target.value) && setSettings({...settings,theme:{...settings.theme,[key]:e.target.value}})} className="min-w-0 w-full border rounded-lg p-2 text-xs font-normal" /></div></label>)}</div>
          <div className="grid sm:grid-cols-3 gap-4"><label className="text-xs font-bold">نمط الأزرار<select value={settings.theme?.buttonStyle || 'rounded'} onChange={(e)=>setSettings({...settings,theme:{...settings.theme,buttonStyle:e.target.value as any}})} className="mt-2 w-full border rounded-xl p-2.5 text-sm font-normal"><option value="rounded">مستدير</option><option value="soft">ناعم</option><option value="pill">بيضاوي</option></select></label><label className="text-xs font-bold">المسافات<select value={settings.theme?.spacingPreset || 'comfortable'} onChange={(e)=>setSettings({...settings,theme:{...settings.theme,spacingPreset:e.target.value as any}})} className="mt-2 w-full border rounded-xl p-2.5 text-sm font-normal"><option value="compact">مضغوطة</option><option value="comfortable">مريحة</option><option value="airy">واسعة</option></select></label><label className="text-xs font-bold">عرض المحتوى<select value={settings.theme?.maxWidthPreset || 'standard'} onChange={(e)=>setSettings({...settings,theme:{...settings.theme,maxWidthPreset:e.target.value as any}})} className="mt-2 w-full border rounded-xl p-2.5 text-sm font-normal"><option value="narrow">ضيّق</option><option value="standard">قياسي</option><option value="wide">واسع</option></select></label></div>
        </div>
