...
import { Archive, ArrowDown, ArrowUp, Copy, Edit3, Eye, Layers3, Plus, RotateCcw, Save, X } from 'lucide-react';
import { BlockType, ContentStatus, Course, Lesson, LessonBlock } from '../../types';
import { coursesService, lessonsService, mediaService, ownerCmsService } from '../../services';
...
export const OwnerLessonBuilder: React.FC = () => {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      setEditingId(null); setForm(blankForm); await loadBlocks();
  ...
  return <section className="space-y-6" dir="rtl">
    <div><h2 className="text-2xl font-black text-slate-900">منشئ محتوى الدروس</h2><p className="text-sm text-slate-500 mt-1">كتل مرتبة قابلة للمعاينة والتكرار والأرشفة، وهي نفس البنية التي سيولدها AI لاحقًا.</p></div>
    ...
    <form onSubmit={save} className="bg-white border rounded-3xl p-5 space-y-4">
      ...
      <textarea value={form.text} onChange={(event) => setForm({ ...form, text: event.target.value })} placeholder="محتوى الكتلة" rows={4} className="w-full border rounded-xl p-3 text-sm"/>
