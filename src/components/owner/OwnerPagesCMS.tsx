...
export const OwnerPagesCMS: React.FC = () => {
  ...
  return <section className="space-y-6" dir="rtl">
    ...
    <form onSubmit={save} className="bg-white border rounded-3xl p-5 space-y-4">
      ...
      <div className="flex gap-2"><button disabled={busy} className="px-5 py-3 bg-indigo-600 text-white rounded-xl text-sm font-black flex gap-2"><Save className="w-4 h-4"/>حفظ كمسودة</button>{editing.id&&<button type="button" onClick={()=>{setEditing(emptyPage);setBodyText('');}} className="px-4 py-3 bg-slate-100 rounded-xl text-sm font-bold">إلغاء</button>}</div>
    </form>
    <div className="grid md:grid-cols-2 gap-4">{pages.map((page)=><article key={page.id} className="bg-white border rounded-2xl p-5 space-y-3"><div className="flex justify-between gap-3"><div><h3 className="font-black">{page.titleAr}</h3><p className="text-xs text-slate-500">/{page.slug} · {page.pageType}</p></div><span className="text-[10px] px-2 py-1 rounded-full bg-slate-100 h-fit">{page.status}</span></div><div className="flex flex-wrap gap-2"><button onClick={()=>select(page)} className="px-3 py-2 border rounded-lg text-xs font-bold">تعديل</button><button onClick={()=>setPreview(page)} className="px-3 py-2 border rounded-lg text-xs font-bold flex gap-1"><Eye className="w-4 h-4"/>معاينة</button>{page.status!=='published'&&page.status!=='archived'&&<button disabled={busy} onClick={()=>void status(page,'published')} className="px-3 py-2 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-bold">نشر</button>}{page.status!=='archived'?<button disabled={busy} onClick={()=>void status(page,'archived')} className="p-2 bg-slate-100 rounded-lg"><Archive className="w-4 h-4"/></button>:<><button disabled={busy} onClick={()=>void status(page,'draft')} className="px-3 py-2 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-bold flex gap-1"><RotateCcw className="w-4 h-4"/>استعادة</button><button disabled={busy} onClick={()=>void permanentDelete(page)} className="p-2 bg-red-50 text-red-700 rounded-lg" title="حذف نهائي للمالك فقط"><Trash2 className="w-4 h-4"/></button></>}</div></article>)}</div>
